<template>
  <div class="chart-container">
    <el-row>
      <label>显示任务条数：</label>
      <el-radio-group v-model="querytaskNum" size="small">
        <el-radio-button :label="item.val" v-for='(item,index) in taskNums'>{{item.label}}</el-radio-button>
      </el-radio-group>
    </el-row>
    <br />
    <el-row class="chart">
      <chart width='100%' ref='piecharts' :options="barChartOption"></chart>
    </el-row>
    <br />
    <el-row>
      <label>查询条件：</label>
      <el-radio-group v-model="queryCondition" size="small">
        <el-radio-button :label="item.val" v-for='(item,index) in conditions'>{{item.label}}</el-radio-button>
      </el-radio-group>
    </el-row>
    <br />
    <el-row>
      <chart width='100%' ref='barcharts' :options="lineChartOption"></chart>
    </el-row>
