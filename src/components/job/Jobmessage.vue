<template>
  <div class="container">
    <el-row>
      <Search @search='onSearch' :autocomplete='true' :list='searchList' :searchval='searchVal' />
    </el-row><br>
    <el-row>
      <el-form label-width="0px" :model="searchForm" :inline="true">

        <el-form-item class="itemPosition">
          <el-input size="small" v-model="searchForm.search" placeholder="--------自定义检索--------" @change="filterList"></el-input>
        </el-form-item>
        <el-form-item class="itemPosition">
          <el-select v-model="searchForm.category" size="small" @change='filterList'>
            <el-option label="自定义任务分类--全部" value="all"></el-option>
            <el-option :label="item" :value="item" v-for='(item,i) in categoryList' :key='i'></el-option>

          </el-select>
        </el-form-item>
        <el-form-item class="itemPosition">
          <el-select v-model="searchForm.exeStatus" @change='filterList' size="small">
            <el-option v-for='(item,index) in exeStatusList' :key='index' :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item class="itemPosition">
          <el-select v-model="searchForm.jobStatus" @change='filterList' size="small">
        ...
        <el-form-item class="itemPosition">
          ...
          <el-date-picker size="small" v-model="searchForm.time" :picker-options="pickerOptions" type="datetimerange" start-placeholder="开始日期" end-placeholder="结束日期" value-format='yyyy-MM-dd HH:mm:ss' @change='filterList' @input="filterList">
          </el-date-picker>
        </el-form-item>
        <el-form-item class="itemPosition">
          <el-button type="primary" size="small" @click="addNew">新增</el-button>
          <el-button type="primary" size="small" @click="batchUpload">批量上传</el-button>
        </el-form-item>
      </el-form>
    ...
    <el-row>
      <el-form :inline="true">
        <el-form-item>
          <el-button-group>
            <el-button size="small" type="primary" :disabled="multipleSelection.length<1" @click='batchHandle("1")'>批量恢复</el-button>
            <el-button size="small" type="danger" :disabled="multipleSelection.length<1" @click='batchHandle("0")'>批量暂停</el-button>
            <el-button size="small" type="danger" :disabled="multipleSelection.length<1" @click='batchHandle("2")'>批量删除</el-button>
            <el-button size="small" type="primary" :disabled="multipleSelection.length<1" @click='batchHandle("3")'>批量导出</el-button>
          </el-button-group>
        </el-form-item>
      </el-form>
    ...
    <el-row>
      <Table :tableData='tableData' :list='tableList' @selection='tableSelectionChange' />
    </el-row>
    <el-row>
      <Pagination v-model:pageSize='pageSize' v-model:currentPage='currentPage' :total='total' @init='init' />
      <el-col>
    ...
    <el-dialog :title="dialogTitle" v-model:visible="dialogTableVisible">
      <div v-if='showType=="edit"'>
        <EditJob :rows='editData' @close='closeDialog' />
      </div>
      <div v-if='showType=="statistics"'>
        <el-row>
          <div class="radigroupBtn">
            <el-radio-group v-model="queryCondition" size="small" @change="changeQueryCondition">
              <el-radio-button :label="item.val" v-for='(item,index) in conditions'>{{item.label}}</el-radio-button>
            </el-radio-group>
          </div>

          <chart width='100%' ref='linecharts' :options="lineOption"></chart>
        </el-row>
      </div>
